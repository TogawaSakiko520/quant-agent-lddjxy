...
class Contract(BaseModel):
    ...
    # 禁止字段重新赋值并拒绝未定义字段；嵌套列表/字典仍可变，调用方不能原地改写快照。
    # Pydantic 构造/校验入口执行下面的验证器；model_copy(update=...) 不重验更新字段，
    # 默认还是浅副本。因此复制对象并不等于重新校验，也不自动复制嵌套状态容器。
    ...
    def require_utc(cls, value: Any, info: ValidationInfo) -> Any:
        """拒绝非 UTC 时间戳及空白的 _id 字段；其他字段按原值返回。"""
...
class DemoConfig(Contract):
    ...
    # 限价相对原始报价的缓冲比例：买入加、卖出减，并参与资金/换手预留。
    ...
    def validate_limits(self) -> Self:
        """拒绝基础目标权重超过单票上限，或历史开始日不早于结束日的配置。"""
        # 基础目标不能主动违反单票上限。
...
class AccountSnapshot(Contract):
    ...
    # 内部账本与 Broker 各自维护这类快照供独立对账；模型本身不记账，也不负责挂单预留。
    # 挂单资金/风险占用由 portfolio/risk 在实际快照之上另算，不能写回成已发生成交。
    # 账户身份必须和执行授权一致。
    account_id: str = "DEMO"
    # 事实查询时刻由注入时钟提供。
    as_of: datetime
...
class TargetPortfolio(Contract):
    ...
    # 由 build_portfolio 生成、plan_orders 读取；positions 描述最终想持有的数量，
    # 不包含当前成交进度。as_of/nav 保留决策时点，不能用执行时刷新覆盖原批准目标。
    decision_id: str
    as_of: datetime
    # 决策时净值，作为换手统一分母。
    nav: Decimal
...
class OrderRecord(Contract):
    ...
    # intent 保留原始身份与委托要求；status/filled_quantity 是随事件或查询更新的投影。
    # 订单投影可先于成交事件到达，所以累计量只用于核对，不能直接据此扣现金/加持仓。
    intent: OrderIntent
    ...
    def validate_status_quantity(self) -> Self:
        """拒绝超量成交及与 OPEN/PARTIAL/FILLED 等状态矛盾的累计数量。"""
        # 成交量不得超过原始委托数量。
...
def canonical_hash(value: Any) -> str:
    """为 JSON 可编码值计算稳定 SHA-256 摘要；不可编码或非有限值沿用 JSON 异常。"""
...
def seal_record(record: RecordT) -> RecordT:
    ...
    # model_copy 不重新跑模型验证；这里仅替换刚计算的哈希字符串，其他字段来自调用方
    # 已验证的记录。封印证明内容一致，不会把 model_copy 造出的非法字段自动变合法。
    return record.model_copy(update={"content_hash": digest})


def verify_record(record: StampedRecord) -> None:
    """重新计算记录内容指纹；缺失或不匹配抛 ContractError，不自动重签输入。"""
...
class EventStore(Protocol):
    ...
    def apply(self, event: OrderEvent | FillEvent) -> bool:
        """原子处理事件；同投递或同成交重复返回 False，新状态事件留痕返回 True。

        不同 event_id 封装的同序号状态事件可返回 True，不代表新增成交或经济影响。
        """
        # 事件身份冲突必须失败而非静默覆盖。
        ...
    ...
    def account_lock(self) -> ContextManager[None]:
        """取得本机同账户执行锁，竞争失败抛 RiskBlocked，退出时释放；不覆盖其他主机或人工操作。"""
        # 账户执行锁独立于每个运行目录。
        ...

    def apply_action(self, action: CorporateAction, at: datetime) -> bool:
        """以 UTC 处理时刻 at 核验公司行动并幂等入账；首次返回 True、重复返回 False，写存储。"""
        # 公司行动与成交共用唯一账务写入边界。
        ...
...
class JournalEntry(Contract):
    ...
    def validate_operation(self) -> Self:
        """确保日志操作与载荷类型对应，避免回放将其他数据当作成交；错配抛 ValueError。"""
...
def equivalent_values(left: Any, right: Any) -> bool:
    ...
    # 顺序列表保留业务固定排序，不擅自忽略次序；先比长度再逐位置 zip，不能让较短
    # 列表截断比较后误报一致。递归只继续比较原结构，不排序或转换字段值。
