...
## 移除 GitHub CI（2026-09-20）

按用户决定删除 `.github/` 的工作流及目录维护说明，并在 `.gitignore` 忽略该目录；同步当前状态及维护说明。保留本地检查、全部测试和独立复核要求，历史 CI 记录仍按原时点保留。

uv 完整检查 439 项通过，demo、validate、replay 与独立复核通过；本轮证据保存于本地 `artifacts/remove-github-ci-20260920/`。

## 文档维护准则补强（2026-09-20）

补充规则自身的维护原则与文档编辑前后的判断方法，区分文档、注释和业务维护路径；同步接手、实现及复核提示，清理当前状态中的旧任务限制。保持既有测试、风控和权限约束，不增加检查器或机械评分。uv / Conda 各 439 项测试及离线 demo、validate、replay 通过，独立复核通过；详情及保留限制见[补强记录](docs/audit/documentation-guidance-20260920.md)。
